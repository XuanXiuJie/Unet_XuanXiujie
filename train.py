...
if __name__ == "__main__":
    ...
    Cuda = True
    #----------------------------------------------#
    #   Seed    在深度学习模型的训练过程中，很多参数如权重和偏置通常是初始化为随机数值的。
    #           通过设置随机种子，可以确保每次运行时这些随机数保持一致，从而使得实验结果具有可比性和可复现性。
    #           使得每次独立训练都可以获得一样的结果
    #----------------------------------------------#
    seed            = 11
    #---------------------------------------------------------------------#
    ...
    #               混合精度训练是一种结合使用单精度浮点数（float）和半精度浮点数（float16）的训练方法。
    #               这种方法可以在保持计算效率的同时，减少模型训练过程中的内存占用，从而提高模型的性能和效率。
    #---------------------------------------------------------------------#
    fp16            = True
    #-----------------------------------------------------#
    ...
    if distributed:
        ...
        # 获取当前进程的本地排名（local rank）
        local_rank = int(os.environ["LOCAL_RANK"])
        # 获取当前进程的全局排名（global rank）
        rank = int(os.environ["RANK"])
        # 设置当前进程使用的设备为对应的GPU
        device = torch.device("cuda", local_rank)
        # 如果是主进程（local rank为0），则打印相关信息
        if local_rank == 0:
            print(f"[{os.getpid( )}] (rank = {rank}, local_rank = {local_rank}) training...")
            print("Gpu Device Count : ", ngpus_per_node)
    else:
        # 如果不使用分布式训练，则设置设备为可用的GPU或CPU
    ...
    if pretrained:
        # 如果使用分布式训练
        if distributed:
            # 如果是主进程（local rank为0），则下载预训练权重
            if local_rank == 0:
                download_weights(backbone)
            # 等待所有进程完成下载
            dist.barrier( )
        else:
            # 如果不使用分布式训练，直接下载预训练权重
            download_weights(backbone)

    # 创建Unet模型，并设置为训练模式
    model = Unet(num_classes = num_classes, pretrained = pretrained, backbone = backbone).train( )

    # 如果没有使用预训练权重，对模型进行初始化
    if not pretrained:
        weights_init(model)

    # 如果指定了模型路径，则加载预训练权重
    if model_path != '':
        # 如果是主进程（local rank为0），则打印加载权重的信息
        if local_rank == 0:
        ...
        # 如果是主进程（local rank为0），则打印成功加载和未成功加载的Key信息
        if local_rank == 0:
